...
Run: `typedframes check examples/sql_connectors/sqlalchemy/`
...
import pandas as pd
from sqlalchemy import Column, ForeignKey, Integer, create_engine, select, text
...
class Order(Base):
    ...
    # Physical column is "order_id" (matches orders.sql's raw-SQL column name below);
    # the ORM attribute stays "id" for brevity, same idea as Customer.cust_id.
...
def load_via_select_inline() -> None:
    ...
    engine = create_engine("postgresql://user:pw@localhost:5434/mydb")
...
def _seed_database() -> None:
    """Drop/recreate the schema from the declarative models above and insert fixture rows.

    Not part of the checker demo -- just enough setup for the functions above to have
    ...
    with Session(engine) as session:
        order1 = Order(amount=150.00)
        order2 = Order(amount=89.50)
        ...
        session.add(Customer(count=3))
        session.commit()


def load_with_unknown_column() -> None:
    """A real bug: 'items' is the relationship attribute, not a real database column.
...
if __name__ == "__main__":
    _seed_database()
    load_via_select_inline()
    load_via_select_variable_and_label()
    load_via_select_bare_model_is_untracked()
    load_via_text_literal()
    load_via_text_from_file()
